...
GS.Editor.prototype.registerEvents = function () {
    ...
    $(window).on('beforeunload', function (event) {
        ...
        if (this.helpers.checkIfUnsavedChanges()) {
            ...
            // For once, WebKit isn't the asshole, but Firefox is.
    ...
    var _getSavedLogo = function (logoId, _cb) {
        ...
        $.get(this.core.config.api.get.logos.userLogo + logoId, function (data) {
            ...
            if (window.Raven) {
                ...
                console.log('!! Raven not found, not setting tags');
            }
...
GS.Editor.Core.prototype.getLogosByCategory = function(queryId, queryPage, queryFilter) {
    $.get(this.config.api.get.logos[queryFilter] + queryId + '/' + queryPage).done(function(data) {
        var category;
        var pagination = {
            current: data.data.page,
            total: data.data.max,
...
GS.Editor.Core.prototype.getSvgCode = function(url, ev) {
    $.ajax({
        ...
        success: function(svg) {
            ...
            var str = xmlSerializer.serializeToString(svg.documentElement);

            var $editor = $(this.config.el.editor);

            // Some common...oddities that may occur in the source :/
            str = str.replace(/NaN/g, '0');
            str = str.replace(/-?Infinity/g, '0');
            ...
            if (viewBox.w > $editor.width() || viewBox.h > $editor.height()) {
                ...
                try {
                    ...
                    for (i = 0; i < transformedNodes.length; ++i) {
                        ...
                        console.log(this.helpers.buildTransform(transform));
...
GS.Editor.Sidebar.prototype.registerEvents = function() {
    ...
    $body.on('click', '[data-logo-pagination]', function(ev) {
        ...
        if ($this.hasClass('logo-listing__filter')) {
            $('.logo-listing__filter').removeClass('logo-listing__filter--active');
            $(this).addClass('logo-listing__filter--active');
        }
...
GS.Editor.Sidebar.prototype.expandActionsSidebar = function() {
    $(this.config.el.editorActionsSidebar).css({ height: window.innerHeight - $(this.config.el.sidebarMenus.logo).outerHeight() });
    ...
    this.defaults = {
        ...
        templates: {
            sidebar: {
                ...
                images: {
                    name: 'sidebar-image-listing.mst',
                    target: '.menu--images .menu__content__inner'
                },
                logos: {
                    name: 'sidebar-logo-listing.mst',
                    target: '.logo-listing'
                },
                search: {
                    name: 'sidebar-logo-search-results.mst',
                    target: '.logo-listing'
                }
            },
            svg: {
...
GS.Editor.Templates.prototype.fetchAndRenderLogosByKeywordTemplate = function(searchData) {
    var paginationData = this.getPaginationData(searchData.pagination.lastPage, 5, searchData.pagination.current);

    var _constructPaginationTemplate = function(paginationData) {
        var finalHtml = '';
        var currentHtml = '';
        var pageClass = 'logo-listing__pagination__page';
        var pageActiveClass = 'logo-listing__pagination__page--active';
        var pageGapFirstClass = 'logo-listing__pagination__page-gap--first';
        var pageGapLastClass = 'logo-listing__pagination__page-gap--last';
         ...
         */
        if (paginationData.previous !== false) {
            currentHtml = _singlePageTemplate(paginationData.current - 1, pageClass, "PREVIOUS");
        }
         ...
         */
        if (paginationData.next !== false) {
            currentHtml = currentHtml + _singlePageTemplate(paginationData.current + 1, pageClass, "NEXT");
        }
    ...
    var _renderLogosByKeywordTemplate = function(template) {
        ...
        if (paginationData.total > 1) {
            $(this.config.templates.sidebar.search.target).find('.logo-listing__pagination__page__wrapper').append(_constructPaginationTemplate(paginationData));
        }
...
GS.Editor.Templates.prototype.fetchAndRenderLogosTemplate = function(categoryData) {
    var paginationData = this.getPaginationData(categoryData.pagination.lastPage, 5, categoryData.pagination.current);
    var _constructFiltersTemplate = function(category) {
        var logoFilters = [
            '<li data-logo-pagination data-logo-category-id="' + category.id + '" data-logo-page="0" data-logo-filter="normal" class="logo-listing__filter">ALL</li>',
            '<li data-logo-pagination data-logo-category-id="' + category.id + '" data-logo-page="0" data-logo-filter="popular" class="logo-listing__filter">POPULAR</li>',
            '<li data-logo-pagination data-logo-category-id="' + category.id + '" data-logo-page="0" data-logo-filter="newLogos" class="logo-listing__filter">NEW</li>',
            '<li data-logo-pagination data-logo-category-id="' + category.id + '" data-logo-page="0" data-logo-filter="random" class="logo-listing__filter">RANDOM</li>'
        ];

        if (category.children) {
            $.each(category.children, function(index, child) {
                logoFilters.push('<li data-logo-pagination data-logo-category-id="' + child.id + '" data-logo-page="0" data-logo-filter="normal" class="logo-listing__filter">' + child.name + '</li>');
            });
        }
    ...
    var _constructPaginationTemplate = function(paginationData) {
         ...
         */
        if (paginationData.previous !== false) {
            currentHtml = _singlePageTemplate(categoryData.category.active, categoryData.filter, paginationData.current - 1, pageClass, "PREVIOUS");
        }
        ...
        if (paginationData.current > 3 && paginationData.max <= paginationData.total) {
            if (paginationData.total > 5) {
                ...
                currentHtml = currentHtml + _singlePageTemplate(categoryData.category.active, categoryData.filter, 1, finalPageClass);
            }
        }
        ...
        for (var i = paginationData.min; i <= finalMaxPages; i++) {
            ...
            currentHtml = currentHtml + _singlePageTemplate(categoryData.category.active, categoryData.filter, i, finalPageClass);
        }
        ...
        if (paginationData.current < paginationData.total - 2 && paginationData.max !== paginationData.total) {
            ...
            currentHtml = currentHtml + _singlePageTemplate(categoryData.category.active, categoryData.filter, paginationData.total, finalPageClass);
        }
         ...
         */
        if (paginationData.next !== false) {
            currentHtml = currentHtml + _singlePageTemplate(categoryData.category.active, categoryData.filter, paginationData.current + 1, pageClass, "NEXT");
        }
    ...
    var _renderLogosTemplate = function(template) {
        ...
        $(this.config.templates.sidebar.logos.target)
            .html(rendered)
            .addClass('logo-listing--active')
            .find('.logo-listing__filter__wrapper')
        ...
        if (paginationData.total > 1) {
            $(this.config.templates.sidebar.logos.target).find('.logo-listing__pagination__page__wrapper').append(_constructPaginationTemplate(paginationData));
        }
...
GS.Editor.Templates.prototype.createNewMasterSvgElementWithData = function(data) {
    ...
    var _renderNewMasterSvgElement = function(template) {
        ...
        $(this.config.el.canvas).prepend($oldDefs);
    ...
    // Storing old defs element
    var $oldDefs = $(this.config.el.canvas).find('> defs');
...
GS.Editor.Toolbars.Text.prototype.prepareTextToolbar = function(toolbarType) {
    ...
    this.colourpickerSolid = ColorPicker($(this.config.el.colourpicker.text.solid)[0], function(hex, hsv, rgb) {
    ...
    this.strokeColourpickerSolid = ColorPicker($(this.config.el.colourpicker.effects.stroke.solid)[0], function(hex, hsv, rgb) {
        $text.attr('stroke', hex);
    }.bind(this));
...
GS.Editor.Toolbars.Shape.prototype.prepareShapeToolbar = function(toolbarType) {
    ...
    this.colourpickerSolid = ColorPicker($(this.config.el.colourpicker.shape.solid)[0], function(hex, hsv, rgb) {
        $(this.config.el.editor).trigger(GS.Editor.Events.Toolbars.UpdateSolidFills, [{ type: toolbarType, parent: $toolbarParent, hex: hex }]);

        $target.attr('fill', hex);
    }.bind(this));

    this.colourpickerGradientFirst = ColorPicker($(this.config.el.colourpicker.shape.gradient.first)[0], function(hex, hsv, rgb) {
    ...
    this.colourpickerGradientSecond = ColorPicker($(this.config.el.colourpicker.shape.gradient.second)[0], function(hex, hsv, rgb) {
        var partFill = this.helpers.getItemActivePart().attr('fill');
        var $gradientStops = $(this.helpers.getGradientIdFromPath(partFill)).find('stop');
...
GS.Editor.Toolbars.Image.prototype.prepareImageToolbar = function(toolbarType) {
    ...
    // Initializing the colourpickers
    $el = $(this.config.el.colourpicker.image.solid);
    if ($el.length > 0) {
        this.colourpickerSolid = ColorPicker($el[0], function (hex, hsv, rgb) {
    ...
    $el = $(this.config.el.colourpicker.image.gradient.first);
    if ($el.length > 0) {
        this.colourpickerGradientFirst = ColorPicker($el[0], function (hex, hsv, rgb) {
            var partFill = this.helpers.getItemActivePart().attr('fill');
            var $gradientStops = $(this.helpers.getGradientIdFromPath(partFill)).find('stop');
            var hexSecond = $(this.config.el.colourpicker[toolbarType].toolbarParent).find(this.config.el.toolbarComponents.fill.gradient.value.second).text();
    ...
    $el = $(this.config.el.colourpicker.image.gradient.second);
    if ($el.length > 0) {
        this.colourpickerGradientSecond = ColorPicker($el[0], function (hex, hsv, rgb) {
            var partFill = this.helpers.getItemActivePart().attr('fill');
            var $gradientStops = $(this.helpers.getGradientIdFromPath(partFill)).find('stop');
            var hexFirst = $(this.config.el.colourpicker[toolbarType].toolbarParent).find(this.config.el.toolbarComponents.fill.gradient.value.first).text();
    ...
    // Initializing the stroke colourpicker
    $el = $(this.config.el.colourpicker.effects.stroke.solid);
    if ($el.length > 0) {
        this.strokeColourpickerSolid = ColorPicker($el[0], function (hex, hsv, rgb) {
            var targetHex = this.helpers.getItemActivePart().attr('stroke', hex);
        }.bind(this));
    }
    ...
    if (isLogin === true) {
        ...
        $.post(this.config.settings.loginUrl, {
            ...
            if (data.success === true) {
                ...
                if (window.Raven && data.user_id) {
                    Raven.setUserContext({
                        id: data.user_id,
                        email: data.email || null
                    });
                    Raven.captureMessage('User logged in: ' + (data.user_id || 0));
                }
            ...
            if (data.success === true) {
                ...
                $.post(this.config.settings.loginUrl, {
                    ...
                    if (window.Raven && data.user_id) {
                        ...
                        Raven.captureMessage('User registered: ' + email);
                        Raven.captureMessage('User logged in: ' + data.user_id || 0);
                    }
...
GS.Editor.Payment = function (options, helpers, templates, core) {
    ...
    this.defaults = {
        ...
        settings: {
            loginUrl: '/auth/login',
            registerUrl: '/account/register',
            paymentTemplateUrl: '/builder/payment?v=1',
            paymentPreviewTemplateUrl: '/builder/payment-preview',
...
GS.Editor.Importer.prototype.registerEvents = function() {
    ...
    $(this.config.el.editor).on(GS.Editor.Events.Core.Draw.SavedLogo, function(ev, data) {
         ...
         *
         * And yes, there is way more commented code in here than there is code, fuck you.
         */
    ...
    $(this.config.el.editor).on(GS.Editor.Events.Importer.Loaded.Business, function(ev, $el) {
        ...
        $(self.config.el.triggers.importerBusiness).val(text);
        ...
        if ($('[data-target-tagline="' + id + '"]').length === 0) {
            var $tagline = $(
                [
                    ...
                    '    <button class="menu--identity__button--apply"></button>',
                    '    <button class="menu--identity__button--remove-tagline"></button>',
                    '    <input value="' + text + '" data-target-tagline="' + id + '" class="menu--identity__input menu--identity__input--tagline" type="text" placeholder="Your tagline">',
                    '  </fieldset>',
                    '</form>'
                ].join('')
            );
    ...
    $(this.config.el.triggers.importerBusiness).on('keydown', function(ev) {
        if (ev.keyCode == self.helpers.keys.KEYENTER) {
            ...
            function updateBusinessText() {
                $(self.config.el.business.text).text($input.val());
...
GS.Editor.Helpers = function(config) {
    this.defaults = {
        settings: {
            fontPath: '/builder_assets/fonts64/'
        },
        attributes: {
...
GS.Editor.Helpers.prototype.embedFonts = function(callMeBack) {
    ...
    var _processFonts = function($text, fontFamily, fontFile, data) {
        var oldText = $svg.find('style').text();
        // @todo: add weight + style
        var fontCode = '@font-face {' +
            'font-family: "' + fontFamily + '";' +
            'src: ' + data + ';' +
        '}';

        $svg.find('> defs style').text(oldText + fontCode);
    ...
    $svg.find('text').each(function(index) {
        ...
        var fontFilename = 'font-' +
            fontFamily.replace(/\s+/g, '-').toLowerCase() + '-' +
            fontWeight + '-' +
            fontStyle;
        var font64 = self.config.settings.fontPath + fontFilename + '.txt';
        var fontTtf = '/builder_assets/fontsttf/' + fontFilename + '.ttf';
        var fontFamilyEmbed = fontFamily + '--' + fontWeight + '--' + fontStyle;
        ...
        $text.attr('font-family', fontFamilyEmbed);

        currentIndex++;

        // Cached or not?
        if (typeof self.fontCache[fontFilename] != 'undefined') {
            _processFonts($text, fontFamilyEmbed, font64, self.fontCache[fontFilename]);
        } else {
            // Synchronous, otherwise fonts may or may not load in time.
            $.ajax({
                url: font64,
                async: false,
                dataType: 'text',
                success: function(data) {
                    self.fontCache[fontFilename] = data;

                    _processFonts($text, fontFamilyEmbed, font64, data);
                }
            });
        }
    });
...
GS.Editor.Onboarding.prototype.doSetupStages = function() {
    this.stages[0] = {
        ...
        content: {
            ...
            description: 'Get started by entering your business name and tagline. Tip: You can edit your name and tagline at any time from the Identity tab in the left navigation.<br>',
            button: 'NEXT'
        },
        action: function() {
    ...
    this.stages[1] = {
        ...
        content: {
            label: 'STEP 2',
            title: 'Select an Image Category.',
            description: 'Scroll through our categories and select what best suits your business. Tip: Select "Staff Favourites" to see some popular images.<br>',
            button: 'NEXT'
        },
        action: function() {
            $body
    ...
    this.stages[2] = {
        ...
        content: {
            label: 'STEP 3',
            title: 'Pick your logo & here we go!',
            description: 'Browse through our logos and pick your favorite. Tip: You can change your logo at any time from the Images tab in the left navigation.<br>',
            button: 'OK, I GOT IT!'
        },
        action: {}
    };
...
GS.Editor.Onboarding.prototype.doStage = function(stageIndex) {
    ...
    switch (stageIndex) {
        case 0:
            this.helpers.submitGaEvent(GS.Editor.Events.GA.Stage1);
            break;
        case 1:
            this.helpers.submitGaEvent(GS.Editor.Events.GA.Stage2);
            break;
        case 2:
            this.helpers.submitGaEvent(GS.Editor.Events.GA.Stage3);
            break;
    }

    var stage = this.stages[stageIndex];
    var og = this.config.el.onboardingGuide;

    // Populating the guide with the data
    $(og.label).text(stage.content.label);
    $(og.title).text(stage.content.title);
    $(og.description).html(stage.content.description);
    $(og.button).text(stage.content.button);
    ...
    // Initiate the stage
    $(this.stages[stageIndex].target).trigger('click');

    $body.on('click', og.button, function() {
...
GS.Editor.Onboarding.prototype.doStart = function() {
    ...
    setTimeout(function() {
        ...
        setTimeout(function() {
            ...
            $(this.config.el.onboardingGuide.main).addClass(this.config.coreClasses.onboardingGuideActive);
        }.bind(this), 500);
    }.bind(this), 300);
};
...
GS.Editor.Onboarding.prototype.doEnd = function() {
    ...
    $('.' + this.config.coreClasses.onboardingActive).one('click', '.logo-listing__logo', function() {
        $body
